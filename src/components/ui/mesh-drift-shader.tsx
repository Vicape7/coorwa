...
// recoloured for Coorwa. Zero dependencies: one WebGL canvas that fills its
// parent. Drop it behind your content with
// <div className="relative"><ShaderBackground className="absolute inset-0" />…
//
// Two changes on the way into this project:
//   1. The recipe colours are Coorwa's, sampled from the logo - see UNIFORMS.
//   2. It honours prefers-reduced-motion: the field renders once, then holds,
//      and the pointer interaction is switched off. An animated background is
...
/**
 * The recipe. Colours are Coorwa's, measured out of the logo file instead of
 * picked by eye: cream, the two highlight ambers, the body amber, and a warm
 * brown heading toward the chocolate chips. Five stops, so `colorCount` is 5
 * and the rest of the array just repeats the last one.
 *
 * Everything else is the builder's "Mesh drift" preset with four values pulled
 * back, because the original was tuned for a dark page:
 *
 *   - hue rotation off. It was set to 6.2657, a full 2π turn, which does
 *     nothing at all.
 *   - vignette down to 0.10, so the corners do not go muddy on cream.
 *   - grain roughly halved.
 *   - timeScale down to about a third. At the original -0.86 the field visibly
 *     churns, and a background that competes with the headline is not premium.
 */
const UNIFORMS = {
  colors: [
    [1, 0.984313725490196, 0.9490196078431372], // #fffbf2 near-white
    [1, 0.8627450980392157, 0.5803921568627451], // #ffdc94 the logo's highlight
    [0.9411764705882353, 0.7215686274509804, 0.3764705882352941], // #f0b860 the logo's body
    [0.8784313725490196, 0.5490196078431373, 0.17254901960784313], // #e08c2c its shadow side
    [0.6901960784313725, 0.396078431372549, 0.12549019607843137], // #b06520 toward the chips
    [0.6901960784313725, 0.396078431372549, 0.12549019607843137],
    [0.6901960784313725, 0.396078431372549, 0.12549019607843137],
    [0.6901960784313725, 0.396078431372549, 0.12549019607843137],
  ] as [number, number, number][],
  colorCount: 5,
  // Larger pools than the preset's 1.46, so each colour reads as its own region rather than
  // averaging into one wash with its neighbours.
  scale: 1.15,
  // The single most important value here. `shade()` places one gaussian per colour and intensity
  // sets how far apart their centres orbit; at the preset's 0.66 they overlap enough that the
  // field flattens to a diagonal gradient, and a flat backdrop is exactly what a lens cannot show.
  intensity: 1.05,
  paramA: 0.21,
  warp: 0.276,
  detail: 2.176,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // A background that moves on its own is the textbook case for this query.
    // Still renders - it just holds one frame and stops listening to the pointer.
    const calm = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const timeScale = calm ? 0 : UNIFORMS.timeScale;
    const cursorEnabled = UNIFORMS.cursorEnabled && !calm;
    ...
    // `render` and `requestRender` call each other, so both stay hoisted function declarations -
    // and TypeScript will not carry the null guards above into a hoisted declaration, since it
    // cannot know when one gets called. Bind the already-checked values once instead.
    const view = canvas;
    const ctx = gl;
    ...
    function render(now: number) {
      ...
      ctx.uniform4f(uni.space, UNIFORMS.offsetX, UNIFORMS.offsetY, mouseX, mouseY);
      ctx.uniform4f(
        uni.cursor,
