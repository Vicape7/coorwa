...
export default async function PairPage({ params }: PageProps<"/terminal/[slug]">) {
  ...
  const found = await resolve(slug);
  if (!found) notFound();
  return found.pair ? <PairView initial={found.pair} /> : <CurvePairView initial={found.curve!} />;
}
