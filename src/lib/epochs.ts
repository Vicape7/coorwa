...
export interface Accrual {
  /** Fees the wallet generated by trading, before the split is applied. */
  traderFeesUsd: Map<string, number>;
  /** Fees earned on tokens the wallet launched, before the split is applied. */
  creatorFeesUsd: Map<string, number>;
...
export function entitlementsFrom(accrual: Accrual): EntitlementLine[] {
  const { traderFeesUsd, creatorFeesUsd, committedUsd, cookPriceUsd } = accrual;
  if (!(cookPriceUsd > 0)) throw new Error("a COOK price is needed to convert a USD balance");

  const wallets = new Set([...traderFeesUsd.keys(), ...creatorFeesUsd.keys()]);
  const out: EntitlementLine[] = [];

  for (const wallet of wallets) {
    const traderUsd = (traderFeesUsd.get(wallet) ?? 0) * CASHBACK_SPLIT.trader;
    const creatorUsd = (creatorFeesUsd.get(wallet) ?? 0) * CASHBACK_SPLIT.creator;
...
export async function computeEntitlements(
  ...
  const [asTrader, asCreator, committed] = await Promise.all([
    conn
      .select({ wallet: fills.wallet, feeUsd: sql<number>`sum(${fills.feeUsd})` })
      .from(fills)
      .where(lte(fills.createdAt, asOf))
      .groupBy(fills.wallet),
    conn
      .select({ wallet: fills.creator, feeUsd: sql<number>`sum(${fills.feeUsd})` })
      .from(fills)
    ...
    conn
      ...
      .where(
        and(
          ...
          or(isNotNull(claims.signature), isNotNull(claims.claimedAt), gt(epochs.deadline, new Date())),
        ),
      )
  ...
  return entitlementsFrom({
    traderFeesUsd: numbers(asTrader.map((r) => ({ wallet: r.wallet, value: r.feeUsd }))),
    creatorFeesUsd: numbers(asCreator.map((r) => ({ wallet: r.wallet, value: r.feeUsd }))),
...
export async function claimableFor(wallet: string | null): Promise<ClaimableReport> {
  ...
  if (heal.length > 0) {
    await Promise.all(
      heal.map((h) => db!.update(claims).set({ claimedAt: h.claimedAt }).where(eq(claims.id, h.id))),
    ).catch(() => undefined);
  }
...
export async function overview(): Promise<EpochOverview> {
  ...
  const chain = deployed ? await fetchEpochs(connection, MINT, stored.map((e) => e.index)) : [];
