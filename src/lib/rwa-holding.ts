 ...
 *
 * Returns 1 for any mint without the extension, and for any read that fails: a wrong multiplier is
 * worse than an unscaled one, and 1 is the only value that is honest about knowing nothing.
 */
export async function readMultiplier(conn: Connection, mint: string): Promise<number> {
  try {
