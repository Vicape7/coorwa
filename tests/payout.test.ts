/**
 * A launchpad creator's fees taken as a stock: the decisions made before the first signature, and
 * what the claim leg hands the bridge.
 *
...
// --- Fees taken as a stock -------------------------------------------------------------------------

test("a claim counts only the fees, not the wrapped COOK its closed account already held", () => {
  const rent = 2_039_280n;
  // No wrapped account before: the claim opened and closed one, so the gain is the fees net of fee.
  assert.equal(claimedCook(3n * COOK - 5_000n, 0n), 3n * COOK - 5_000n);
  // The wallet already held 2 wrapped COOK: closing the account released it with its rent.
  assert.equal(claimedCook(5n * COOK + rent - 5_000n, 2n * COOK + rent), 3n * COOK - 5_000n);
});

test("a payout bridges the claimed COOK plus the sale, and a costly claim is charged to it", () => {
  assert.equal(claimToBridge(3n * COOK, 4n * COOK, RESERVE), 7n * COOK - RESERVE);
  // Opening a token account can cost more COOK than the claim paid, and that is counted.
  assert.equal(claimToBridge(-2_000_000n, 4n * COOK, RESERVE), 4n * COOK - 2_000_000n - RESERVE);
  assert.ok(claimToBridge(RESERVE / 2n, 0n, RESERVE) <= 0n);
});

test("each pool resumes on its own", () => {
  assert.notEqual(creatorPayoutSlug("poolA"), creatorPayoutSlug("poolB"));
});

test("fees under the floor are sent back to a plain claim, in their own words", () => {
